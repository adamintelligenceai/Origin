...
test('marketing home renders live engine figures', async ({ page }) => {
  ...
  await expect(page.getByRole('main').getByRole('link', { name: 'Run the demo scan' })).toBeVisible();
  await expect(page.getByRole('main').getByText('Detected leakage', { exact: true })).toBeVisible();
