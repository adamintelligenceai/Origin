...
**Why not 10.** Generated `.docx`, `.xlsx`, `.pptx`, and `.pdf` now exist from `tools/build_*.py`. Field use by a live Controller has still not happened.
...
**Why not 10.** No live adapter has been configured from these files. Field untested.

**Closed in this build:** `.docx`, `.xlsx`, and `.pptx` binaries are generated by `tools/build_docx.py`, `tools/build_spreadsheets.py`, and `tools/build_pptx.py`.
